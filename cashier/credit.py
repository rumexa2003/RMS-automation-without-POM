
from playwright.sync_api import sync_playwright, expect

with sync_playwright() as p:
    browser = p.chromium.launch(headless=False)
    page = browser.new_page()

    page.goto("https://rms.geckoworksnepal.com.np/staff/login")

    table_input = page.get_by_placeholder("GECKO-01")
    table_input.fill("GC-01")

    expect(table_input).to_have_value("GC-01")

    page.get_by_role("button", name="Activate System").click()
    page.get_by_role("button", name="Ramala").click()
    page.wait_for_timeout(1000)  

    for i in range(4):
        page.get_by_role("button", name="0").click()
    page.wait_for_timeout(1000)  

    page.get_by_role("button", name="Start Shift").click()
    page.wait_for_timeout(1000)  
  
    payment_pending = page.locator("button").filter(
    has_text="payment_pending"
    )

    payment_pending.click()
    page.wait_for_timeout(5000)  

    # Exact matching
    page.get_by_role("button", name="Credit", exact=True).click()
    page.wait_for_timeout(5000) 

    # amount = page.locator('input[placeholder="0"][min="0"]')
    # amount.fill("100")  

    # page.wait_for_timeout(5000)  
    # page.wait_for_timeout(5000)  
    # page.get_by_role("button", name="Add to Bill", exact=True).click()
    # page.wait_for_timeout(5000)  


    customer = page.get_by_placeholder(
    "* Select or Type Customer Name"
    )

    customer.fill("John Doe")

    page.wait_for_timeout(5000)  
    page.wait_for_timeout(5000)  



    confirm_payment = page.get_by_role(
    "button",
    name="Confirm Payment"
    )

    expect(confirm_payment).to_be_visible()
    expect(confirm_payment).to_be_enabled()

    confirm_payment.click()
    


    # page.get_by_role("button", name="Kitchen Status").click()
    
    # page.get_by_role("button", name="Serve Ready Items").click()
    page.wait_for_timeout(5000)  

    

    browser.close()
